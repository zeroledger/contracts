...
import {WithdrawItem} from "src/Vault.types.sol";
...
contract VaultWithdrawTest is VaultTest, IVaultEvents {
  ...
  uint240 constant WITHDRAW_AMOUNT_1 = uint240(50e18);
  uint256 constant S_VALUE_1 = uint256(123456789);

  uint240 constant WITHDRAW_AMOUNT_2 = uint240(30e18);
  uint256 constant S_VALUE_2 = uint256(111111111);

  uint240 constant WITHDRAW_AMOUNT_3 = uint240(20e18);
  uint256 constant S_VALUE_3 = uint256(222222222);

  uint240 constant WITHDRAW_AMOUNT_4 = uint240(10e18);
  uint256 constant S_VALUE_4 = uint256(333333333);

  // Deposit constants
  uint240 constant DEPOSIT_AMOUNT = uint240(100e18);
  uint240 constant DEPOSIT_FEE = uint240(5e18);
  ...
  function test_withdraw_single_commitment_success() public {
    ...
    createDeposit(alice, DEPOSIT_AMOUNT, DEPOSIT_FEE, depositHashes, depositOwners);
    ...
    uint240 withdrawFee = uint240(1e18);
    ...
    vm.expectEmit(true, true, false, true);
    emit Withdrawal(alice, address(mockToken), WITHDRAW_AMOUNT_1, withdrawFee);

    // Execute withdraw
    vm.startPrank(alice);
    vault.withdraw(address(mockToken), items, recipient, withdrawFee, feeRecipient);
    vm.stopPrank();
  ...
  function test_withdraw_multiple_commitments_success() public {
    ...
    uint240 withdrawFee = uint240(2e18);
    address recipient = alice;
    ...
    vm.expectEmit(true, true, false, true);
    emit Withdrawal(alice, address(mockToken), WITHDRAW_AMOUNT_2 + WITHDRAW_AMOUNT_3, withdrawFee);
  ...
  function test_withdraw_invalid_token() public {
    ...
    vault.withdraw(address(0), items, recipient, withdrawFee, feeRecipient);
    vm.stopPrank();
  }
  ...
  function test_withdraw_fee_greater_than_amount() public {
    ...
    uint240 withdrawFee = uint240(20e18); // Fee greater than amount
    address recipient = alice;
