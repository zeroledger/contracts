...
async function getWitnessCalculator(directory: string) {
  const out = path.join(__dirname, "../circuits/circuits", directory, "build");

  let wc;
  const wasmPath = path.join(out, `${directory}_js/${directory}.wasm`);

  if (!witnessCalculatorCache.has(wasmPath)) {
    const witnessCalculator = (await import(path.join(out, `${directory}_js/witness_calculator.js`))).default;
...
export async function prove(input: any, directory: string) {
  ...
  const { proof, publicSignals } = await snarkjs.plonk.prove(path.join(out, `${directory}.zkey`), wtns);
  ...
  console.log(`Total time: ${(witnessTime + proveTime + vkeyTime + verifyTime).toFixed(2)}ms`);
...
export async function exportSolidityCallData(proof: any, publicSignals: any) {
  ...
  const proofArr: any = match[1].split(",").map((val: string) => val.trim().replace(/"/g, ""));
  const pubSignalsArr: any = match[2].split(",").map((val: string) => val.trim().replace(/"/g, ""));
