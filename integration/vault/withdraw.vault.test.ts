...
describe("Vault Withdraw Tests", function () {
    describe("Successful Withdrawals", function () {
        ...
        it("should successfully withdraw multiple commitments", async function () {
            ...
            const totalWithdrawn = amounts.reduce(
                (sum, amount) => sum + amount,
                0n
            );

            expect(finalBalances.user).to.equal(
                initialBalances.user + totalWithdrawn
            );
            expect(finalBalances.vault).to.equal(
                initialBalances.vault - totalWithdrawn
            );
        ...
        it("should successfully withdraw from a complex deposit", async function () {
            ...
            expect(finalBalances.user).to.equal(
                initialBalances.user + withdrawAmount
            );
            expect(finalBalances.vault).to.equal(
                initialBalances.vault - withdrawAmount
            );

            // Verify the withdrawn commitment was removed
            await verifyCommitmentRemoved(
                commitmentData.hashes[0],
                vault,
                mockToken
            );

            // Verify other commitments still exist
            const commitment1 = await vault.commitmentsMap(
                await mockToken.getAddress(),
                commitmentData.hashes[1]
            );
            const commitment2 = await vault.commitmentsMap(
                await mockToken.getAddress(),
                commitmentData.hashes[2]
            );
    ...
    describe("Failure Cases", function () {
        it("should fail when withdrawing non-existent commitment", async function () {
            const { vault, mockToken, user } = await loadFixture(
                deployVaultFixture
            );
            ...
            await expect(
                vault.connect(testData.user).withdraw(
                    ...
                    [
                        {
                            amount: testData.amount,
                            sValue: `0x${Buffer.from(randomBytes(32)).toString(
                                "hex"
                            )}`,
                        },
                    ],
        ...
        it("should fail when withdrawing with wrong sValue", async function () {
            ...
            const wrongSValue = `0x${Buffer.from(randomBytes(32)).toString(
                "hex"
            )}`;

            // Create a commitment first
            await createCommitmentForWithdraw(
                amount,
                user,
                vault,
                mockToken,
        ...
        it("should fail when withdrawing commitment owned by different user", async function () {
            const { vault, mockToken, user, otherUser, feeRecipient } =
