...
contract VaultSpend11Test is VaultTest, IVaultEvents {
  ...
  function test_spend11_success() public {
    ...
    // Execute the spend transaction (silent = false to emit events)
    vm.startPrank(alice);
    vault.spend(transaction, proof, false);
    vm.stopPrank();
  ...
  function test_spend11_silent() public {
    // Setup: Create a deposit first to establish commitments
    uint240 depositAmount = uint240(100e18);
    uint240 depositFee = uint240(0); // No deposit fee
    uint240 spendFee = uint240(5e18);
    ...
    // Expect CommitmentRemoved and CommitmentCreated events, but NOT ConfidentialSpend
    vm.expectEmit(true, true, false, true);
    emit CommitmentRemoved(alice, address(mockToken), inputHash);
    ...
    // Execute the spend transaction with silent = true (should not emit ConfidentialSpend)
    vm.startPrank(alice);
    vault.spend(transaction, proof, true);
    vm.stopPrank();
