...
describe("Vault Spend12 Tests", function () {
    describe("Successful Spends", function () {
        it("should successfully spend 1 input into 2 outputs with valid ZK proof", async function () {
            ...
            const commitmentData = await generateSpendCommitmentData(
                depositCommitmentData,
                testData.outputAmounts
            );
            ...
            const tx = await vault
                ...
                .spend(transaction, calldata_proof);
            ...
            expect(finalBalances.vault).to.equal(
                initialBalances.vault - testData.publicOutputs[0].amount
            );
            expect(finalBalances.feeRecipient).to.equal(
                initialBalances.feeRecipient + testData.publicOutputs[0].amount
            );

            // Verify input commitment was removed
            await verifyInputCommitmentsRemoved(
                commitmentData.inputHashes,
                vault,
                mockToken
            );
            ...
            await verifyOutputCommitmentsCreated(
                commitmentData.outputHashes,
